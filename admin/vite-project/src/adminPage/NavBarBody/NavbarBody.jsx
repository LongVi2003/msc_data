...
import {
  Button, Modal, Form, Input, List, Row, Col, Card, Select, message
} from 'antd';
...
const NavbarBody = () => {
  ...
  const handleSaveChart = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/charts", {
        prompt: customPrompt,
        type: chartType,
      });
  ...
  const fetchVendors = async (fieldCategory) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/fields/${fieldCategory}/vendors`);
  ...
  const interpretPrompt = (prompt) => {
    ...
    if (lower.includes("tròn") || lower.includes("pie") || lower.includes("tỉ trọng") || lower.includes("phần trăm")) return "pie";
    ...
    <Col>
        ...
        <Col span={10}>
          <Card title="Cài đặt lĩnh vực">
            <List
              ...
              renderItem={item => (
                <List.Item actions={[
                  <Button danger type="link" onClick={() => handleDelete(item.name)}>Xoá</Button>
                ]}>
                  <strong>{item.name}</strong> - {item.value}%
                </List.Item>
              )}
            />
            <Button type="primary" block onClick={openModel} style={{ marginTop: 16 }}>Thêm lĩnh vực</Button>
          </Card>

          <Card title="Lấy danh sách Vendor theo lĩnh vực" style={{ marginTop: 24 }}>
            <Form layout="inline">
      ...
      >
        <Form form={form} onFinish={handleAddField} layout="vertical">
          <Form.Item name="name" label="Tên Lĩnh Vực" rules={[{ required: true, message: 'Vui lòng nhập tên' }]}>
            <Input />
          </Form.Item>
          <Form.Item
            name="value"
